...
class PlayState extends MusicBeatState
{
	...
	public static var groundFront:FlxSprite;
	...
	var col:Array<FlxColor> = [
		...
		0xFF142447 // AFALC BUT HES GENERALLY ANGRY FOR ABSOLUTELY NO REASON 2
	];
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'chorus':
				dialogue = CoolUtil.coolTextFile(Paths.txt('chorus/chorusDialogue'));
		}
			...
			case 'loaf' | 'trance':
				{
					...
					loaf = new FlxSprite(-190, 700);
					loaf.frames = Paths.getSparrowAtlas('village/LOAF');
					loaf.animation.addByPrefix('bop', "loaf", 24, false);
					loaf.antialiasing = true;
					loaf.setGraphicSize(Std.int(loaf.width * 1.8));
					loaf.updateHitbox();
					loaf.antialiasing = false;
					add(loaf);
				}
			case 'blazeborn':
				{
			...
			case 'the-end':
				{
					...
					var skybox = new FlxSprite(0, 0).loadGraphic(Paths.image('village/skybox'));
					skybox.antialiasing = true;
					skybox.scrollFactor.set(0.6, 0.6);
					skybox.active = false;
					skybox.scale.x += 1;
					skybox.scale.y += 1;
					add(skybox);

					bg = new FlxSprite(0, 0).loadGraphic(Paths.image('village/villagebgnight'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.6, 0.6);
					bg.active = false;
					bg.scale.x += 1;
					...
					groundFront = new FlxSprite(0, 200).loadGraphic(Paths.image('village/fore'));
					groundFront.setGraphicSize(Std.int(groundFront.width * 1.1));
					groundFront.updateHitbox();
					groundFront.antialiasing = true;
					groundFront.scrollFactor.set(0.9, 0.9);
					groundFront.active = false;
					groundFront.scale.x += 1;
					groundFront.scale.y += 1;
					add(groundFront);
					// camera.bgColor = 0xFF7dabff;
			...
			case 'chorus':
				{
					defaultCamZoom = 0.7;
					curStage = 'void';
					var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('void/bg'));
					bg.antialiasing = true;
					...
					var island:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('void/endislands'));
					island.antialiasing = true;
					island.scrollFactor.set(0.6, 0.6);
					island.scale.x += 1;
					island.scale.y += 1;
					add(island);

					var stageFront:FlxSprite = new FlxSprite(0, 800).loadGraphic(Paths.image('void/front'));
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					stageFront.antialiasing = true;
					stageFront.scrollFactor.set(0.9, 0.9);
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				gfVersion = 'gf-pixel';
			case 'overworld' | 'overworldnight' | 'overworldp3' | 'void':
				gfVersion = 'gf-mc';
		}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.x += 180;
				gf.y += 300;
			case 'overworld' | 'overworldnight' | 'void':
				boyfriend.x += 200;
				dad.x -= 70;
				dad.y += 70;
				gf.scale.x -= 0.05;
		...
		var kadeEngineWatermark = new FlxText(4, healthBarBG.y + 50, 0, ("VSAflac (subscribe to aflac)"));
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						if (health <= 0.2)
							EndingSubState.Ending = 1;
						if (health >= 0.2)
							EndingSubState.Ending = 0;
						FlxG.switchState(new EndingSubState());
					}
					else
	...
	override function beatHit()
	{
		...
		if (curSong == 'the-end') // This is much more compact to do
		{
			switch (curBeat)
			{
				case 32:
				...
				case 178: // 178
					...
					FlxTween.tween(bg, {y: 700}, 2, {ease: FlxEase.smoothStepInOut});
					boyfriend.playAnim('hit', true);

				case 306 | 315 | 322 | 331: // smol shake
					camera.shake(0.06, 0.4, null, true, FlxAxes.X);
					FlxG.camera.zoom = 1.1;
					var agressive:Bool = true;
				case 312 | 320 | 328 | 336: // beeg shakes
					camera.shake(0.09, 0.7, null, true, FlxAxes.X);
				case 500:
					camera.shake(0.01, 10, null, true, FlxAxes.XY);
				case 516:
					camGame.fade(FlxColor.WHITE, 4, false);
			}
		}

		if (curSong == 'blazeborn')
		{
			if (curBeat == 19 || curBeat == 51 || curBeat == 83 || curBeat == 115 || curBeat == 147)
				camera.shake(0.01, 0.4, null, true, FlxAxes.X);
			if (curBeat == 179 || curBeat == 212)
				camera.shake(0.02, 0.4, null, true, FlxAxes.X);
			if (curBeat == 78 || curBeat == 95 || curBeat == 103 || curBeat == 111)
				camera.shake(0.005, 0.4, null, true, FlxAxes.XY);
		}
		switch (curStage)
		{
			case 'school':
